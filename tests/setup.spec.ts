...
base.describe('Setting up the testing environment', {
    annotation: {
        type: 'Setup',
        description: 'Environment setup tests - run manually'
    }
}, () => {
    base.skip(({ browserName }) => true, 'Setup tests should be run manually with: npx playwright test tests/setup.spec.ts --project=chromium');

    base('Enable_multiple_admin_logins', { tag: ['@setup', '@override'] }, async ({ page, browserName }, testInfo) => {
    ...
    base('Disable_login_captcha', { tag: ['@setup', '@override'] }, async ({ page, browserName }, testInfo) => {
        const browserEngine = browserName?.toUpperCase() || "UNKNOWN";
        const magentoAdminUsername = requireEnv('MAGENTO_ADMIN_USERNAME');
        const magentoAdminPassword = requireEnv('MAGENTO_ADMIN_PASSWORD');
    ...
    base('Setup_environment_for_tests', { tag: ['@setup', '@override'] }, async ({ page, browserName }, testInfo) => {
        const browserEngine = browserName?.toUpperCase() || "UNKNOWN";
